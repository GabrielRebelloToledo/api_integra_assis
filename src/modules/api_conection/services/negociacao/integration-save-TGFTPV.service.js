...
import TGFTPV from '../../../../entities/tgftpv.entities.js';
...
import fieldTGFTPV from '../fields/tgftpv.field.js'

class IntegrationServiceTGFTPV {
    constructor() {
        // Repositório do TypeORM para a entidade Userx

        this.getRepositoryEtities = AppDataSource.getRepository(TGFTPV);
    }
    ...
    async execute() {
        ...
        let qtdepaginas = 1
        if (registro.qtvenda > 0) {
            qtdepaginas = registro.qtvenda;
        }
        const serviceLoad = container.resolve(LoadsRecords);

        console.log("Excluindo TGFTPV");
        await this.getRepositoryEtities.delete({});

        console.log("Adicionando TGFTPV")
        try {
            for (let i = 0; i <= qtdepaginas; i++) {
                try {
                    const registroLoad = await serviceLoad.records('TipoNegociacao', i, fieldTGFTPV.join(','));
                    ...
                    for (const registro of registroLoad) {
                        ...
                        fieldTGFTPV.forEach((field, index) => {
...
export default IntegrationServiceTGFTPV;
