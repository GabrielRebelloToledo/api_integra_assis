...
import TGFMBC from '../../../../entities/tgfmbc.entities.js';
...
import fieldTGFMBC from '../fields/tgfmbc.field.js'

class IntegrationServiceTGFMBC {
    constructor() {
        // Repositório do TypeORM para a entidade Userx

        this.getRepositoryEtities = AppDataSource.getRepository(TGFMBC);
    }
    ...
    async execute() {
        ...
        let qtdepaginas = 1
        if (registro.qtmbc > 0) {
            qtdepaginas = registro.qtmbc;
        }
        const serviceLoad = container.resolve(LoadsRecords);

        console.log("Excluindo TGFMBC")
        await this.getRepositoryEtities.delete({});

        console.log("Adicionando TGFMBC")
        try {
            for (let i = 0; i <= qtdepaginas; i++) {

                try {

                    const registroLoad = await serviceLoad.records('MovimentoBancario', i, fieldTGFMBC.join(','));
                    ...
                    for (const registro of registroLoad) {
                        ...
                        fieldTGFMBC.forEach((field, index) => {
...
export default IntegrationServiceTGFMBC;
