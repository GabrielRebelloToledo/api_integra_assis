...
import TSICTA from '../../../../entities/tsicta.entities.js';
...
import fieldTSICTA from '../fields/tsicta.field.js'

class IntegrationServiceTSICTA {
    constructor() {
        // Repositório do TypeORM para a entidade Userx

        this.getRepositoryEtities = AppDataSource.getRepository(TSICTA);
    }
    ...
    async execute() {
        ...
        let qtdepaginas = 1
        if (registro.qtcta > 0) {
            qtdepaginas = registro.qtcta;
        }
        const serviceLoad = container.resolve(LoadsRecords);

        console.log("Excluindo TSICTA")
        await this.getRepositoryEtities.delete({});

        console.log("Adicionando TSICTA")
        try{
            for (let i = 0; i <= qtdepaginas; i++) {

                try {
                    const registroLoad = await serviceLoad.records('ContaBancaria', i, fieldTSICTA.join(','));
                    ...
                    for (const registro of registroLoad) {
                        ...
                        fieldTSICTA.forEach((field, index) => {
