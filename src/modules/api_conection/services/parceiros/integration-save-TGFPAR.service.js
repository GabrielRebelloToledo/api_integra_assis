...
import TGFPAR from '../../../../entities/tgfpar.entities.js';
...
import fieldTGFPAR from '../fields/tgfpar.field.js';
class IntegrationServiceTGFPAR {
    constructor() {
        // Repositório do TypeORM para a entidade Userx

        this.getRepositoryEtities = AppDataSource.getRepository(TGFPAR);
    }
    ...
    async execute() {
        ...
        let qtdepaginas = 1
        if (registro.qtparc > 0) {
            qtdepaginas = registro.qtparc;
        }
        ...
        console.log("Adicionando Parceiros")
        try {
            for (let i = 0; i <= 4; i++) {

                try {
                    const registroLoad = await serviceLoad.records('Parceiro', i, fieldTGFPAR.join(','));
                    ...
                    for (const registro of registroLoad) {
                        ...
                        fieldTGFPAR.forEach((field, index) => {
                        ...
                        const savos = await this.getRepositoryEtities.save(registros);
                    }
...
export default IntegrationServiceTGFPAR;
