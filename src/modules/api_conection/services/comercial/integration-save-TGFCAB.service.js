...
import TGFCAB from '../../../../entities/tgfcab.entities.js';
...
import fieldTGFCAB from '../fields/tgfcab.field.js'

class IntegrationServiceTGFCAB {
    constructor() {

        this.getRepositoryEtities = AppDataSource.getRepository(TGFCAB);
    }
    ...
    async execute() {
        ...
        let qtdepaginas = 1
        if (registro.qtcab > 0) {
            qtdepaginas = registro.qtcab;
        }
        const serviceLoad = container.resolve(LoadsRecords);

        console.log("Excluindo TGFCAB")
        await this.getRepositoryEtities.delete({});

        console.log("Adicionando TGFCAB")
        try {
            for (let i = 0; i <= qtdepaginas; i++) {

                try {

                    const registroLoad = await serviceLoad.records('CabecalhoNota', i, fieldTGFCAB.join(','));
                    ...
                    for (const registro of registroLoad) {
                        ...
                        fieldTGFCAB.forEach((field, index) => {
...
export default IntegrationServiceTGFCAB;
