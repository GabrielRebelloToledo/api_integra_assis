...
import TSICUS from '../../../../entities/tsicus.entities.js';
...
import fieldTSICUS from '../fields/tsicus.field.js'

class IntegrationServiceTSICUS {
    constructor() {
        this.getRepositoryEtities = AppDataSource.getRepository(TSICUS);
    }
    ...
    async execute() {
        ...
        let qtdepaginas = 1
        if (registro.qtccus > 0) {
            qtdepaginas = registro.qtccus;
        }
        const serviceLoad = container.resolve(LoadsRecords);

        console.log("Excluindo TSICUS")
        ...
        try{for (let i = 0; i <= qtdepaginas; i++) {
            ...
            try {
                ...
                const registroLoad = await serviceLoad.records('CODCENCUS', i, fieldTSICUS.join(','));
                ...
                for (const registro of registroLoad) {
                    ...
                    fieldTSICUS.forEach((field, index) => {
...
export default IntegrationServiceTSICUS;
