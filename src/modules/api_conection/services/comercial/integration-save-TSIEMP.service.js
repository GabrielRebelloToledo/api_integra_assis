...
import TSIEMP from '../../../../entities/tsiemp.entities.js';
...
import fieldTSIEMP from '../fields/tsiemp.field.js'

class IntegrationServiceTSIEMP {
    constructor() {

        this.getRepositoryEtities = AppDataSource.getRepository(TSIEMP);
    }
    ...
    async execute() {
        ...
        let qtdepaginas = 1
        if (registro.qtemp > 0) {
            qtdepaginas = registro.qtemp;
        }
        const serviceLoad = container.resolve(LoadsRecords);

        console.log("Excluindo TSIEMP")
        await this.getRepositoryEtities.delete({});

        console.log("Adicionando TSIEMP")
        try{
            for (let i = 0; i <= qtdepaginas; i++) {

                try {
    
                    const registroLoad = await serviceLoad.records('Empresa', i, fieldTSIEMP.join(','));
                    ...
                    for (const registro of registroLoad) {
                        ...
                        fieldTSIEMP.forEach((field, index) => {
...
export default IntegrationServiceTSIEMP;
